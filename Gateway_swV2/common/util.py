...
class GenericApp():
    ...
    def __init__(self, connection, apis):
        ...
        self.log = logging.getLogger('Farmpro')
        self._run = False

        cl = logging.StreamHandler()
        cl.setLevel(logging.INFO)

        # fl = logging.FileHandler(filename="log/farmprolog_{:%Y%m%d}.log".format(datetime.datetime.now()), encoding="utf-8")
        fl = logging.FileHandler(filename="farmprolog.log")
        fl.setLevel(logging.INFO)
        
        # 하루에 한 번 새로운 log 파일 생성
        # timedfilehandler_info = logging.handlers.TimedRotatingFileHandler(filename="log/farmprolog_{:%Y%m%d}.log".format(datetime.datetime.now()), 
        #                                                                     when='midnight', 
        #                                                                     interval=1,
		# 													                encoding='utf-8')
        ...
        self.log.addHandler(cl)
        # self.log.addHandler(fl)
        # self.log.addHandler(timedfilehandler_info)
...
def get_connector(conn=None):
    ...
    if conn is None:
        ...
        # return bgapi.SerialConnector("/dev/ttyS2")
        return bgapi.SerialConnector("/dev/serial0")
    else:
...
def CheckAddress(hostaddr, addr):
    mac = addr.replace(':','')
    int_mac = int(mac, 16)
    bytes_mac = bytearray.fromhex('{:012x}'.format(int_mac))
    if(hostaddr != bytes_mac):
        return 0
    return 1